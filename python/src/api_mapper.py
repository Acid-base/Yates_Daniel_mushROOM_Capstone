...
class MOApiMapper:
    ...
    async def discover_ids(self):
        ...
        response = await self._make_request(
            "observations",
            {"names": "Agaricus", "detail": "low", "has_images": "true"}
        )
        if "results" in response:
            ...
            for obs_id in self.example_ids["observations"]:
                img_response = await self._make_request(
                    "images",
                    {"observation_id": str(obs_id), "detail": "low"}
                )
        ...
        response = await self._make_request(
            "names",
            {"children_of": "Agaricus", "detail": "low"}
        )
        if "results" in response:
    ...
    async def map_endpoints(self):
        ...
        help_endpoints = [
            {
                "endpoint": name,
                "params": {"help": "1"},
                "save_as": f"{name}_help"
            }
            for name in ["observations", "names", "images", "locations", "sequences", "external_links"]
        ]
        ...
        if self.example_ids["names"]:
            ...
            await self.test_endpoint(
                "names",
                {
                    "id": str(self.example_ids["names"][0]),
                    "detail": "high"
                },
                "name_detail_high"
            )
        ...
        if self.example_ids["locations"]:
            ...
            await self.test_endpoint(
                "locations",
                {
                    "id": str(self.example_ids["locations"][0]),
                    "detail": "high"
                },
